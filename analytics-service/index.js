...
async function connectRabbitMQWithRetry(maxRetries = 10, delayMs = 5000) {
  ...
  while (attempt < maxRetries) {
    try {
      console.log(
        `[Analytics] Connecting to RabbitMQ (attempt ${attempt + 1}/${maxRetries}):`,
        RABBITMQ_URL
      );
      ...
      attempt += 1;
      console.error('[Analytics] RabbitMQ connection failed:', err.message);
      if (attempt >= maxRetries) {
        throw new Error('Could not connect to RabbitMQ after multiple attempts');
      }
      console.log(`[Analytics] Retrying in ${delayMs / 1000}s...`);
...
async function start() {
  const conn = await connectRabbitMQWithRetry();
  ...
  channel.consume(
    ...
    async (msg) => {
      ...
      try {
        ...
        console.error(
          '[Analytics] Failed to process user.friendship.changed:',
          err.message
        );
        channel.nack(msg, false, false);
      }
