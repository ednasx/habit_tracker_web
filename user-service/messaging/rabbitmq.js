...
export async function publishFriendshipChangedEvent(event) {
  ...
  try {
    const ch = await ensureChannel()
    if (!ch) {
      console.warn('[RabbitMQ] Channel not ready, cannot publish user.friendship.changed event for userId:', event.userId, 'friendId:', event.friendId)
      return
    }

    const payload = Buffer.from(JSON.stringify(event))
    const published = ch.publish(EXCHANGE_NAME, 'user.friendship.changed', payload, {
      persistent: true,
    })
    
    if (!published) {
      console.warn('[RabbitMQ] Failed to publish user.friendship.changed event (buffer full) for userId:', event.userId, 'friendId:', event.friendId)
    } else {
      console.log('[RabbitMQ] Published user.friendship.changed event for userId:', event.userId, 'friendId:', event.friendId, 'action:', event.action)
    }
  } catch (err) {
    console.error('[RabbitMQ] Failed to publish user.friendship.changed event for userId:', event.userId, 'friendId:', event.friendId, 'Error:', err.message)
  }
}
